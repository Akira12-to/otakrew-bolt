...
import { ArrowLeft, Plus, X, Eye, Edit } from 'lucide-react'
...
export const NewThreadForm: React.FC = () => {
  ...
  const createThreadMutation = useMutation({
    mutationFn: (data: NewThreadData) => forumApi.createThread(categoryId!, data),
    onSuccess: (newThread) => {
      queryClient.invalidateQueries({ queryKey: ['threads', categoryId] })
      navigate(`/forum/${categoryId}/${newThread.id}`)
    }
  })
        ...
        <div className="container-otaku py-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Nouveau sujet</h1>
            ...
            <div className="flex items-center justify-between">
              ...
              <motion.button
                ...
                className={`flex items-center gap-2 px-8 py-3 rounded-lg font-medium transition-all duration-300 ${
                  isValid && !createThreadMutation.isPending
                    ? 'otaku-button hover:shadow-otaku-halo-sm'
                    : 'bg-gray-600 text-gray-400 cursor-not-allowed'
                }`}
                whileHover={isValid && !createThreadMutation.isPending ? { scale: 1.05 } : {}}
                whileTap={isValid && !createThreadMutation.isPending ? { scale: 0.95 } : {}}
              >
